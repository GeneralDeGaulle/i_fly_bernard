...
#%% if needed filtre avion ou flotte
...
df_group_month = (df_all_flights.groupby("month", as_index=False, sort=False)
                  ["co2_emission_tonnes"].sum())

df_group_month_c = (df_all_flights.groupby("month", as_index=False, sort=False)
                  ["registration"].count())

df_group_month_c.loc[:, "year"] = df_group_month_c["month"].str.strip().str[4:]

df_group_year = (df_all_flights.groupby("year", as_index=False, sort=False)
                  ["co2_emission_tonnes"].sum())

df_group_day_c = (df_all_flights.groupby("departure_date_only_utc", as_index=False, sort=False)
                  ["registration"].count())

df_4month = (df_all_flights.groupby(pd.Grouper(key="departure_date_utc", freq="4MS"))
                  [["registration", "co2_emission_tonnes"]].count())
...
sns.barplot(data=df_4month, x="departure_4month", y="registration", color="#3498db")
# palette="Blues_d"
...
sns.barplot(data=df_group_day_c, x="departure_date_only_utc", y="registration")
