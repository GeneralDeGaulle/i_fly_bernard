...
for ac in list_avions:
    ...
    for i, flight in df_filter_ac.iterrows():
        ...
        fig.add_trace(go.Scattermapbox(lon = geo_lon, lat = geo_lat, mode = "lines",
                                            line_width = 4, line_color = flight.color_avion,
                                            name = legend_i))
...
fig.update_layout(margin = {"r":0,"t":0,"l":0,"b":0}, showlegend = True,
                  ...
                  title_text = f"<b>Tous les vols du {date_map} de la compagnie de location de jets privés Valljet</b><br>Total: {flight_temps_str} de vol - {str(co2_tot)} t de CO2 <b>",
                  title_font_family="Arial", title_font_size=25, title_x=0.5, title_y=0.96)

fig.update_layout(legend_font_size=10, legend_borderwidth = 1, legend_bordercolor = "grey",
                  legend_title = f"Vol de la journée du {date_map} - Flotte Valljet", legend_title_font_size = 12,
