...
for aircraft_row in df_avion.itertuples():
    ...
    list_new_flights_ac = []
    list_new_flights_ac = adsb_exchange.fct_adsbex_check_new_flights_and_kml(
        icao24_ac, registration_ac, path_flight_data, last_check_date)
    ...
    if list_new_flights_ac:
        ...
        list_folder_and_kml_paths = kml_to_csv.fct_kml_2_folder(
            list_new_flights_ac, path_flight_data
        )

        # si nouveau(x) vol(s), on continue (en effet, si on lance le programme plusieurs fois par
        # jour, on peut avoir un cas où "list_folder_and_kml_paths" est vide)
        if list_folder_and_kml_paths:
            ...
            for flight_leg_kml in list_folder_and_kml_paths:
                ...
                # on génère tout mais on ne continue que si l'avion a volé.
                # Parfois, ils n'ont qu'une phase au sol (maintenance de l'A/C certainement).
            ...
            if list_new_csv:
                ...
                df_new_flights_only = post_flight_consolidation.fct_short_flight(
                    df_new_flights_only)
                ...
                df_new_flights_only = df_new_flights_only.drop(
                    columns=["departure_date_only_utc_map"])
                ...
                list_print_new_flights.append(
                    f"--- {registration_ac} - {len(df_new_flights_only)} vols générés ---")
                print()
