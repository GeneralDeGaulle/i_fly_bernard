...
def fct_airport_vs_cruise(df_data):
    ...
    if m > 0:
        df_data.loc[condition_cruise_arr, "airport_arrival"] = df_data.loc[
            condition_cruise_arr, "next_flight_departure"]
        df_data.loc[condition_cruise_arr, "airport_arr_icao"] = df_data.loc[
            condition_cruise_arr, "next_flight_departure_icao"]
        df_data.loc[condition_cruise_arr, "iso_country_arr"] = df_data.loc[
            condition_cruise_arr, "next_flight_dep_iso_country"]
        df_data.loc[condition_cruise_dep, "airport_departure"] = df_data.loc[
            condition_cruise_dep, "previous_flight_arrival"]
        df_data.loc[condition_cruise_dep, "airport_dep_icao"] = df_data.loc[
            condition_cruise_dep, "previous_flight_arrival_icao"]
        df_data.loc[condition_cruise_dep, "iso_country_dep"] = df_data.loc[
            condition_cruise_dep, "previous_flight_arr_iso_country"]
    ...
    df_data = df_data.drop(
        columns=[
            ...
            "previous_flight_arrival",
            "next_flight_departure_icao",
            "next_flight_dep_iso_country",
            "previous_flight_arrival_icao",
            "previous_flight_arr_iso_country"])
...
def fct_short_flight(df_data):
    ...
    df_data = df_data[
        (df_data["airport_arrival"] != df_data["airport_departure"])
...
def fct_check_2flights_in1(df_data, output=0):
    ...
    df_data_prep.loc[:, "flight_took_too_long"] = (
        df_data_prep["flight_duration_min"] / 60.0 - df_data_prep["flight_duration_theorique_h"]
    ) > 0.5
    ...
    if nb_of_issues > 0:
        if output == 0:
            print(
                f"!!! {registration_ac} - {nb_of_issues} vols trop long avec un gap de position !!!")
        else:
            print(
                f"!!! {registration_ac} - {nb_of_issues} vols trop long avec un gap de position !!!")
...
def fct_check_reconciliation(df_ac_data):
    ...
    df_vols_tbc_1 = df_vols_tbc[
        (df_vols_tbc["airport_arrival"] == "A/C in cruise")
...
def plot_df_csv(df_csv, save_file_path):
    fig = px.scatter_mapbox(
        ...
        lon="long",
        color="elevation",
        color_continuous_scale="Temps",  # Temps sunset sunsetdark,Plasma_r
