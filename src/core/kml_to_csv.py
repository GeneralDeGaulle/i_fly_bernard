...
def fct_kml_2_folder(flights_legs_list_new, path_f_data):
    ...
    for flight_leg in flights_legs_list_new:
        ...
        if not os.listdir(path_flight_leg_folder):
            ...
            # on sauve les path des fichiers kml et les infos pour faciliter la
            # function suivante "fct_kml_2_csv" dans "kml_to_csv.py"
            list_folder_and_kml_paths.append(
                [path_flight_leg_folder, new_path_kml, file_name_only])
...
def fct_kml_2_csv(flight_leg_kml):
    ...
    for track in kml_soup.find_all("gx:Track"):
        ...
        if track.find("extrude").contents[0] == "1":
            ...
            # on regroupe time avec la précedentes liste créée, dans chaque sous liste de llet.
            # Pas simple mais efficace
            for i, time in enumerate(leg_and_ground_when):
                list_llet_i[i].append(time.contents[0])

        # au cas où plusieurs phases de vols coupées dans le même kml, on concatène
        # toutes les listes (e.g. touch and go)
    ...
    # sinon, on sauvegarde notre csv et on retourne le path_csv qui sera notre point d'entrée
    # pour "get_new_df_data.py"
    else:
