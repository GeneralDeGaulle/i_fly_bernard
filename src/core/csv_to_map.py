...
def fct_csv_2_map(rel_path_flight_csv, regis, date_vol, co2, flight_temps, proprio):
    ...
    # normalement, ce n'est pas utile de repasser en absolu car cwd() est bien définit.
    # Mais pour d'autres users github, ça aidera.
    ...
    if len(df) > 2:
        ...
        bearing = maths_for_bernard.fct_get_bearing(
            center_lat, center_long, center_lat_next, center_long_next)
    ...
    if df_gaps.empty:
        ...
        # comme Plotly ne permet pas d'ajouter ces propres images par dessus scatter_mapbox,
        # on le fait avec PIL. l'astuce trouvée est d'avoir positionner le centre de la trajectoire
        # au centre de l'image. donc on a pas besoin de calculer une fonction de
        # transfert lat/long --> pixel.PIL ajoute l'image de l'avion au centre de l'autre image et
        # c'est bon. ça empêche d'être souple mais au moins c'est automatique et ça marche !
        map_img = Image.open(path_image)
        ac_img = Image.open(path_ac_png)
        ac_img = ac_img.rotate(
            -bearing, resample=Image.BICUBIC
        )  # - because counter clockwise, resamp to no lose quality
    ...
    # si un trou est identifié (ou plusieurs), on complète le graph précédent avec geodesic (graph plotly et pas PIL)
        ...
        fig.update_layout(
            ...
            height=800,
            mapbox_zoom=3,
            mapbox_center=dict(lat=center_lat, lon=center_long))
